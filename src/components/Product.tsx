"use client";

import { products } from "@wix/stores";
import Link from "next/link";
import WixImage from "./WixImage";
import { ArrowUpRight, ShoppingCart } from "lucide-react";

interface ProductProps {
  product: products.Product;
}

export default function Product({ product }: ProductProps) {
  const mainImage = product.media?.mainMedia?.image;
  const description = product.additionalInfoSections?.[0]?.description;

  const isDiscounted =
    product.priceData?.formatted?.price !==
    product.priceData?.formatted?.discountedPrice;

  return (
    <div className="block relative">
      <Link href={`/products/${product.slug}`} className="block">
        <div className="group/card relative w-full h-[420px] flex flex-col overflow-hidden rounded-xl bg-zinc-900 border border-zinc-800 transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl hover:shadow-emerald-500/10 hover:border-emerald-500/30">
          {/* Discount Badge */}
          {isDiscounted && (
            <div className="absolute top-3 left-3 z-20 bg-gradient-to-r from-emerald-500 to-green-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg">
              SALE
            </div>
          )}

          {/* Arrow/Cart Indicator - Top Right */}
          <div className="absolute top-3 right-3 z-20">
            <div className="relative bg-zinc-800 group-hover/card:bg-emerald-500 backdrop-blur-sm rounded-full p-1.5 shadow-lg transition-colors duration-300">
              <ArrowUpRight className="w-3.5 h-3.5 text-white opacity-100 group-hover/card:opacity-0 transition-opacity duration-300" />
              <ShoppingCart className="w-3.5 h-3.5 text-black absolute inset-0 m-auto opacity-0 group-hover/card:opacity-100 transition-opacity duration-300" />
            </div>
          </div>

          {/* Image Container - Fixed height */}
          <div className="relative h-[280px] overflow-hidden flex-shrink-0">
            <WixImage
              height={400}
              width={400}
              mediaIdentifier={mainImage?.url}
              alt={mainImage?.altText}
              className="w-full h-full object-contain transition-all duration-700 group-hover/card:scale-110"
            />

            {/* Gradient Overlay - Always visible on mobile, hover on desktop */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent md:from-black/60 md:via-transparent md:to-transparent opacity-100 md:opacity-0 md:group-hover/card:opacity-100 transition-opacity duration-300" />

            {/* Description Overlay - Always visible on mobile, hover on desktop */}
            {description && (
              <div className="absolute inset-0 flex items-end p-4 opacity-100 md:opacity-0 md:group-hover/card:opacity-100 transition-opacity duration-300">
                <div
                  className="text-sm text-white/90 line-clamp-2 md:line-clamp-3 font-medium leading-relaxed"
                  dangerouslySetInnerHTML={{
                    __html: description,
                  }}
                />
              </div>
            )}
          </div>

          {/* Content Section - Fixed height remainder */}
          <div className="flex-1 flex flex-col justify-between p-4 min-h-[140px]">
            {/* Product Name - Fixed height area */}
            <div className="space-y-2 h-[60px] flex flex-col justify-start">
              <h3 className="font-bold text-lg leading-tight line-clamp-2 text-white group-hover/card:text-emerald-400 transition-colors duration-300">
                {product.name}
              </h3>

              {/* Underline - Always visible on mobile, animated on desktop */}
              <div className="w-8 md:w-0 md:group-hover/card:w-12 h-0.5 bg-emerald-500 transition-all duration-500 ease-out" />
            </div>

            {/* Pricing - Fixed height area */}
            <div className="h-[60px] flex flex-col justify-end">
              <div className="flex items-end justify-between">
                {isDiscounted ? (
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-bold text-emerald-400">
                        {product.priceData?.formatted?.discountedPrice}
                      </span>
                      <span className="text-sm text-zinc-500 line-through">
                        {product.priceData?.formatted?.price}
                      </span>
                    </div>
                    <div className="text-xs text-emerald-500 font-medium">
                      Limited Time Offer
                    </div>
                  </div>
                ) : (
                  <div className="space-y-1">
                    <span className="text-xl font-bold text-white">
                      {product.priceData?.formatted?.price}
                    </span>
                    <div className="text-xs text-zinc-500">Premium Build</div>
                  </div>
                )}

                {/* View Badge - Always visible on mobile, hover on desktop */}
                <div className="opacity-100 md:opacity-0 md:group-hover/card:opacity-100 transition-opacity duration-300">
                  <div className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 px-2 py-1 rounded-md text-xs font-semibold">
                    View
                  </div>
                </div>
              </div>

              {/* Bottom gradient line */}
              <div className="w-full h-px bg-gradient-to-r from-transparent via-emerald-500/30 to-transparent mt-3" />
            </div>
          </div>

          {/* Card glow effect */}
          <div className="absolute inset-0 rounded-xl opacity-0 group-hover/card:opacity-100 transition-opacity duration-500 pointer-events-none -z-10">
            <div className="absolute inset-0 rounded-xl bg-emerald-500/5 blur-xl" />
          </div>
        </div>
      </Link>
    </div>
  );
}
