          ...
          <Link key={cat._id} href={`/collections/${cat.slug}`}>
            <div className="group relative overflow-hidden rounded-2xl aspect-[4/3] cursor-pointer transform hover:scale-105 transition-all duration-500 bg-white border border-gray-200 shadow-lg hover:shadow-2xl">
              ...
              <div className="relative z-10 h-full flex flex-col items-center justify-center text-center p-6">
                ...
                <div className={`w-16 h-16 rounded-full border-2 flex items-center justify-center group-hover:scale-110 transition-all duration-300 ${hasImage ? 'border-white/80 group-hover:border-white bg-white/10 backdrop-blur-sm' : 'border-primary/60 group-hover:border-primary'}`}>
                  <svg className={`w-8 h-8 ${hasImage ? 'text-white' : 'text-primary'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
